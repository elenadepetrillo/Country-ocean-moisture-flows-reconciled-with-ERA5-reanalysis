#BACKWARD BILATERAL MATRIX OF ATMOSPHERIC MOISTURE FLOWS BETWEEN COUNTRIES/OCEANS from the UTrack dataset (https://doi.org/10.1594/PANGAEA.912710, https://github.com/ObbeTuinenburg/UTrack_global_database)
...
This script extracts from the dataset the probability that an precipitation particle at a target cell originates from an evapotranspiration at a source cell s, i.e. a 'backward footprint' (bw_fp).
The bw_fp is given as a probability grid over the whole world, for each target cell t and for each month m. 
For each specific month, the backward footprint bw_fp is multiplied by the days of the month and the precipitation (TP) from ERA5 at a target cell t, to obtain the monthly moisture flux. Further multiplying 
by the area of the target cell, the moisture volumes are obtained. The moisture volumes are Precipitation (TP) in m^3 at target cell t and Evapotranspiration (ET) in m^3 at source cell s.
...
#Number of processors to run the multiprocessing
num_processors = 
...
#load countries and oceans codes and names
...
#function to retrive at the cell scale UTrack fraction of precipitation from evaporation in the backward case
#the function will go in the parallel run
def country_cell_loop(c_cell): 
    lat = lats[c_cell[0]]
    lon = lons[c_cell[1]]
    bw_fp = mf.sel(targetlat=lat, targetlon=lon).values
    mask = np.where(bw_fp == 0)
    bw_fp = bw_fp * (-0.1)
    bw_fp = np.exp(bw_fp)
    bw_fp[mask] = 0
    bw_fp = ET * bw_fp
    bw_fp = bw_fp / np.sum(bw_fp)
    tp = TotalPrec.sel(lat=lat, lon=lon).values * days
    area = cell_area.sel(lat=lat, lon=lon).values
    bw_fp = tp * bw_fp * area
    return bw_fp, area

#define an empty (zeros) matrix
matrix_bw = np.zeros((len(country_ocean_iso), len(country_ocean_iso)))
...
for m in months:
    ...
    days = get_days_in_month(m)
    #Multiply input data of ERA5 ET to have positive values
    ET = (-1) * EvapoTransp.values 
    ...
    for i, c in enumerate(country_ocean_list): #begin cycle over countries/oceans
	    ...
	    #Now cumulated_flux must be subdivided into contributions from -potentally- every country/ocean in the world
            ...
            #fill the column of the matrix as the precipitation of the i- country
            matrix_bw[:, i] += cumulated_flux_by_country #assign the row sum to the output matrix

np.savetxt(file_path, matrix_bw, delimiter=';') #save the output matrix
